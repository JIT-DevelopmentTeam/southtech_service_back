...
public class ItemController extends JeecgController<Item, IItemService> {
    ...
    public <T extends BaseEntity> String synchronization(Class<T> tClass, IService service, String url, String syncType) throws Exception {
        ...
        try {
            ...
            if (dataList.isEmpty() && "2".equals(syncType) && dataArray.size() > 10000) {
                ...
                ThreadRunItem threadRunItem1 = new ThreadRunItem(itemArray1, saveMethod, service, dataList, this);
                ThreadRunItem threadRunItem2 = new ThreadRunItem(itemArray2, saveMethod, service, dataList, this);
                ThreadRunItem threadRunItem3 = new ThreadRunItem(itemArray3, saveMethod, service, dataList, this);
                ThreadRunItem threadRunItem4 = new ThreadRunItem(itemArray4, saveMethod, service, dataList, this);
                ThreadRunItem threadRunItem5 = new ThreadRunItem(itemArray5, saveMethod, service, dataList, this);
                threadRunItem1.run();
                threadRunItem2.run();
                threadRunItem3.run();
                threadRunItem4.run();
                ...
                // 增量同步
                saveItemBatch(tClass, dataArray, saveMethod, list, service, dataList);
            }
        } catch (Exception e) {
            e.printStackTrace();
            String msg = "";
     ...
     * 批量储存商品
     */
    private <T extends BaseEntity> void saveItemBatch(Class<T> tClass, JSONArray dataArray, Method saveMethod, List list, IService service,List<T> dataList) {
        for (int i = 0; i < dataArray.size(); i++) {
